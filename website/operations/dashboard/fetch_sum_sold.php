<?php
    ...
    if (isset($_POST['rarities'])) {
        ...
        if (count($rarities) > 0 && $rarities[0] != ''){
            $sql = "SELECT SUM(item_total_times_sold) AS 'sum' FROM (SELECT * FROM items WHERE item_rarity IN ($rarity_placeholders)) items ";
            $stmt = $dbconnect -> prepare($sql);
            $stmt -> bind_param(str_repeat('s', count($rarities)), ...$rarities);
        }
        else {
            $sql = "SELECT SUM(item_total_times_sold) AS 'sum' FROM items";
            $stmt = $dbconnect -> prepare($sql);
        }
        $stmt -> execute();
