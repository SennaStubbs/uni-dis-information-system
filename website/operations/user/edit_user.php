<?php
    ...
    if (isset($_POST['user_id']) && isset($_POST['user_username']) && isset($_POST['user_password']) && isset($_POST['user_display_name']) && isset($_POST['user_access_level'])) {
        ...
        $_user_access_level = filter_var($_POST['user_access_level'], FILTER_SANITIZE_NUMBER_INT);
    }
    else {
        echo 'error';
        exit();
    ...
    if ($_user_id > 0 && trim($_user_username) != '' && trim($_user_password) != '' && $_user_display_name != '0' && $_user_access_level !== false) {
        ...
        $stmt = "SELECT user_id FROM users WHERE user_username = ? AND user_id != ?";
        $sql = $dbconnect -> prepare($stmt);
        $sql -> bind_param('si', $_user_username, $_user_id);
        $sql -> execute();
        ...
        $stmt = "UPDATE users SET user_username = ?, user_password = ?, user_display_name = ?, user_access_level = ? WHERE user_id = ?";
        $sql = $dbconnect -> prepare($stmt);
        $sql -> bind_param('sssii', $_user_username, $_user_password, $_user_display_name, $_user_access_level, $_user_id);
        $sql -> execute();
        $result = $sql -> get_result();
